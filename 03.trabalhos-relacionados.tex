...
\nota{deu pra entender,mas coloca ou vírgula, ou divide as frases:
baseada na biblioteca flowbag. Essa biblioteca é embalada ou a funcao em python esta embalada em um funcao virtual de rede?}

Na camada de de captura, pacotes de rede são transformadas em fluxos e enviados
...
executadas em dispositivos de rede virtuais.
...
\nota{com o conjunto de dados sintético UFRJ/GTA --> coloca a referencia}
...
\nota{tambem para a avaliacao do que?}
...
\nota{se vc fala de deteccao de anomalia aqui em baixo, nao eh interessante colocar ele no meio da explicacao nos paragrafos anteriores tambem? E outra, deteccao de anomalia ou novidade que ele usa?}

Em conclusão, a ferramenta CATRACA apresenta uma arquitetura de divisão em
camadas alocadas em ambientes de névoa (\fog) e nuvem (\cloud) avaliado com
métricas e dois conjuntos de dados relevantes.
No entanto, o algoritmo de detecção de anomalias consiste de um modelo de
classificação pelo método árvore de decisão e atualização do modelo durante a
fase \emph{Online} depende de todos exemplos do último intervalo de atualização,
portanto não é capaz de reconhecer novidades.
