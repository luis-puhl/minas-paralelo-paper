...
Duas abordagens, detecção por assinatura e dedecção por anomalia, são de uso
frequente como o mecanismo de detecção de intrusão em NIDS.
...
assinatura, devido o tempo de resposta (que evolve a identificação e criação de
uma assinatura) grande demais para prevenir esse tipo de intrusão.

A ferramenta BigFlow é composta por dois módulos: extração de atributos e .
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.8\textwidth]{figuras/bigflow-fig2-feature_extraction_module.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{figuras/bigflow-fig4-reliable_learning_module.png}
...
\citeonline{Viegas2019} destaca que data sets adequados para NIDS são
...
\mathrm{Gbps}$ entre Japão e EUA, com inicio em 2006 continuamente até hoje,
...
classificadores da literatura em dois modos de operação quanto seus dados de
treinamento (ambos contendo uma semana de captura).
...
Comparando os resultados entre os modos de operação, demostra que a qualidade da
...
Manager}) e as multiplas instâncias do serviço gerenciador de tarefas
(\emph{Task Manager}).

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{figuras/bigflow-fig5-bigflow_arch.png}
...
No entanto a atualização semanal e, mais importante, por meio de avaliação de um
...
Também para a avaliação da ferramenta CATRACA, utiliza as métricas de qualidade
...
descritos na \refsec{nd} que são atendidos por algoritmos de detecção de
novidade.
...
Arquitetura \idsiot, proposta por \citeonline{Cassales2019a}, tem por objetivo
monitorar uma rede local com dispositivos \iot e detectar tentativas de intrusão
e até possível subersão destes dispositivos.
...
a quatidade de requisções de classificação por especialista.
...
convecionais produzia exmplos e adicionava como mensagens em um tópico no
...
Ambas avaliações demonstraram o equilibrio entre qualidade de classificação e
velocidade ou uso de recursos.
O algoritmo ECSMiner mostrou melhor qualidade de classificação porém com
velocidade e consumo de recursos próximos de uma ordem de magnitude pior.
...
de recurusos computacionais (memória) foi elevado.
