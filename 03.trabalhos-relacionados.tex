...
baseado em detecção de anomalias, abordagem preferida por esses sistemas em
contraste com a abordagem de detecção por assinatura que tem tempo de resposta
a novos tipos de ataque (\emph{zero day}) grande demais para detectar esse tipo
de intrusão.

A ferramenta BigFlow é composta por dois módulos e múltiplos submódulos.
O módulo de extração de atributos é responsável por transformar pacotes em fluxos
e fluxos em exemplos para o módulo de aprendizado confiável.
...
submódulo de exemplos rejeitados, responsável por requisitar etiquetas para exemplos rejeitados e;
...
\dataset \emph{Packet traces from WIDE backbone, samplepoint-F} composto por seções de captura
de pacotes diárias de 15 minutos de um link de $1 \mathrm{Gbps}$ entre Japão e EUA, com
inicio em 2006 continuamente até hoje, anonimizados e etiquetados por
MAWILab \cite{mawiSamplepointF,Fontugne2010}.
...
% Com o data set MAWIFlow original e reduzido foram avaliados quatro
% classificadores da literatura em dois modos de operação quanto seus dados de
% treinamento (ambos contendo uma semana de captura) o primeiro usando somente a
% primeira semana do ano e as demais como teste e o segundo modo usando a semana
% anterior como treinamento e a seguinte como teste. Demostrando, com 62
% atributos, que a qualidade da classificação retrai com o tempo quando não há
