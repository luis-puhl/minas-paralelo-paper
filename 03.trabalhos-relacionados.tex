...
CATRACA\footnote{
    A ferramenta e sua documentação estão disponíveis em http://gta.ufrj.br/catraca
}.
A ferramenta CATRACA é composta de funções virtuais de rede (\emph{Network
Function Virtualization}, NFV) executadas em dispositivos de de rede virtuais
estabelecendo um ambiente de computação em névoa.
Também compõem a ferramenta uma interface gráfica que apresentada a visualização
dos fluxos processados bem como os conhecimentos extraídos desse processamento.
...
disponíveis. A comparação envolveu:
...
Itens do \dataset UFRJ/GTA eram enviados ao cluster kafka lidos pela plataforma
em teste. 
