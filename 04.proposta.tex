...
O \mfog é composto em módulos, sendo eles: \offline, \classify e \detector.
...
juntamente com os valores do \dataset fornecidos pelo \source, computar
...
as métricas tem como índice o instante $n$, informando a posição do exemplo em
relação ao fluxo.
...
Os resultados desses experimentos contém as seguintes métricas:
...
Para a validação da implementação, as métricas de qualidade de classificação
serão comparadas com as mesmas métricas extraídas do com os mesmo \datasets
de experimentos semelhantes executados com a implementação original do algoritmo
MINAS.

% \nota{AQUI NO PARAGRAFO ANTERIOR, NAOOOOO!! Nao colocar frase depois do
% enumerate. Comeca um paragrafo novo ou continua ele com uma frase nova, nao pode
% fazer isso}
