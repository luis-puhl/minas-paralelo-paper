...
% No entanto, se o valor raio for definido como a distância máxima (Equação
% \ref{eq:raio_max}), se um novo \mcluster é formado apenas por exemplos
% desconhecidos e, todo exemplo desconhecido tem distância ao cluster mais próximo
% maior do que o raio do cluster mais próximo, NÃO SEGUE LOGICAMENTE, o valor de
...
\begin{algorithm}[htb]
  ...
  \Function{\NoveltyDetection{Modelo, Desconhecidos}}{
    ...
    \ForEach{novoCluster $\in$ \clustering(Desconhecidos)}{
      \If{$(|\;novoCluster.exemplos\;| \geq$ \minExamplesPerCluster $) \land (\;novoCluster.silhueta\; > 0)$}{
...
modificando da representação de \cluster, algoritmo de agrupamento, manutenção
...
estre trabalho, ficam somente aqui anotadas para algum trabalho futuro.
As modificações são:
