...
\begin{algorithm}[htb]
    ...
    \Function{\MinasOnline{Modelo, fluxoEntrada}}{
        ...
        \ForEach{ {$exemplo_{i}$} $\in$ fluxoEntrada }{
            % exemplo.etiqueta $\leftarrow$ unknown\;
            ...
            \eIf{maisPróximo.distância $<$ maisPróximo.cluster.raio}{
                exemplo.etiqueta $\leftarrow$ maisPróximo.cluster.etiqueta\;
                maisPróximo.cluster.últimoUso $ \leftarrow i $ \;
            }
            {
                exemplo.etiqueta $\leftarrow$ "desconhecido"\;
...
A segunda operação da fase \emph{Online} é a detecção de padrões novidade, que é
        ...
        \ForEach{novoCluster in \clustering(Desconhecidos)}{
            \If{$|\;novoCluster.exemplos\;| \geq$ \minExamplesPerCluster}{
                ...
                \eIf{maisPróximo.distância $<$ (maisPróximo.cluster.raio $\times$ \noveltyFactor)}{
                    novoCluster.etiqueta $\leftarrow$ maisPróximo.cluster.etiqueta\;
                    novoCluster.tipo $\leftarrow$ "extensão"\;
                }{
                    novoCluster.etiqueta $\leftarrow$ proximaNovidade\;
