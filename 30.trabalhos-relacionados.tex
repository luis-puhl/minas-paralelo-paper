...
Como o mecanismo de detecção de intrusão na construção de um \nids, duas
abordagens são comuns, detecção por assinatura e detecção por anomalia, porém,
para a detecção de novos tipos de ataque (\emph{zero day}) a abordagem de
detecção por anomalia é vantajosa devido ao tempo de resposta grande demais para
...
extrair as características dos desses pacotes formando descritores de fluxo
...
não são ideais para detecção de ameças e respectiva ação sobre a descoberta de
novos padrões.
