...
Nesta \Section, abordam-se trabalhos que aplicam \hl{algoritmos de detecção de
novidades} em \hl{ambiente de processamento distribuído} de fluxo de dados em tempo
real.
...
\notahelio{?}\hl{são de uso frequente,}
\hl{como o mecanismo}
\hl{de detecção de intrusão na construção de NIDS}.
...
é responsável por coletar \hl{pacotes} da rede monitorada,
\notahelio{extrair as características dos desses pacotes... não "transformar pacotes"...}
\hl{transformar esses pacotes em fluxos}
...
centésimo, \hl{mantendo} as proporções de etiquetas (Ataque e Normal), \hl{facilitando} o
...
\notahelio{?}\hl{No entanto, a atualização semanal e, mais importante,}
\hl{dependendo de avaliação de}\\
\hl{um especialista não é ideal}
\hl{para detecção de novidades e}
\hl{respectiva ação sobre a descoberta}\\
\hl{de novos padrões.}
...
\notahelio{por que não?}
\hl{não é capaz de lidar}
\hl{adequadamente com as características de fluxos contínuos} de dados, como os
...
detecção de intrusão entre nós na \notahelio{ou edge?}\hl{rede de borda (\fog)} e nós em nuvem pública
(\cloud).
...
Com as avaliações realizadas, a arquitetura \idsiot \hl{opta por distribuir} as
...
\hl{paralelismo, multi-processamento} ou \hl{distribuição computacional}, que são
...
em cada um dos temas, \hl{falta ainda uma abordagem que estabeleça uma união} entre o
estado da arte em \hl{algoritmos de detecção} de novidade e o estado da arte em
\hl{processamento distribuído} de fluxos de dados, em especial para \hl{o ambiente de
computação em névoa} focado em \hl{fluxos de dados} relacionados a \hl{dispositivos \iot.}
