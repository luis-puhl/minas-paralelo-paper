...
\begin{table}[ht]
  ...
  \begin{tabularx}{\linewidth}{X|X|X|X|X|X}
    ...
    % Murilo Naldi da UFSCAR & plataforma & técnica & \dataset & métricas \\
    % Não encontrei uma plataforma do Naldi, encontrei algoritmos.
    % \hline
    % Latifur Khan & plataforma & técnica & \dataset & métricas \\
    % \hline
    % CLAM & plataforma & técnica & \dataset & métricas \\
    % \hline
    % trabalhos do Bifet e o framework baseado no MOA (samoa) & plataforma & técnica & \dataset & métricas \\
    % \hline
  \end{tabularx}
  \label{tab:summary}
  \end{scriptsize}
...
abordagens são comuns, detecção por assinatura e detecção por anomalia. Para a detecção de novos tipos de ataque (\emph{zero day}) a abordagem de
detecção por anomalia é vantajosa comparada a detecção por assinatura que tem tempo de resposta grande demais para
prevenir esse tipo de intrusão.
...
\citeonline{Viegas2019} destaca que \datasets adequados para \nids são
...
Para avaliar o desempenho de \nids, o \dataset MAWIFlow é proposto por \citeonline{Viegas2019}.
Este \dataset é derivado dos traços de pacotes no \emph{backbone} WIDE, ponto de amostras F
...
\todo{colocar breve resumo de quais metricas foram usadas pelo bigflow, explica-las, e dizer para que serviram}
...
\todo{qual foi o resultado dessa avaliacao?}
...
detecção de intrusão entre nós de névoa (\fog) e nós em nuvem pública (\cloud).
...
Esse \dataset tem as características desejáveis de um conjunto para detecção de
...
paralelismo, distribuição que são necessários para tratar fluxos de dados com
grandes volumes e velocidades.
