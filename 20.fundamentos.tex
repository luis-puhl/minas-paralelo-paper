...
local. Para esse tipo de sistema, o paradigma de programação paralela SPMD (\emph{Single Program, Multiple
Data}; em português, Único Programa, Múltiplos Dados) pode ser implementado utilizando o
conceito de memória distribuída compartilhada através de troca de mensagens.

Em paradigmas de computação paralela, o modelo de passagem de mensagens é amplamente
utilizado para a comunicação de diferentes processos. Nesse modelo, cada
processo tem sua memória local e se comunicam com outros processos através da
troca de mensagens \cite{mpi-book}. Observando boas práticas e melhores funcionalidades ao longo dos anos, 
a comunidade e a indústria padronizaram sistema de passagem de mensagens no padrão MPI (\emph{Message Passing Interface}, em
português Interface de Troca de Mensagens).
...
bibliotecas de troca de mensagens em ambientes de memória compartilhada. Nesse
padrão, cada processo tem seu espaço de endereçamento diferente, e se comunica
com os outros através de mensagens. Esse modelo pode ser implementado desde
multiprocessadores de memória compartilhada, redes workstations até em máquinas
unicore, podendo usar também arquiteturas híbridas (processos
\emph{mulithread} com memória compartilhada, múltiplos processos sem memória
compartilhada, etc.) \cite{mpi-book}. O MPI, por meio de alguma
implementação como OpenMPI e MPICH, permite a construção de um sistema
distribuído com um executável único (monólito) utilizando com abstração a
passagem de mensagens.

COLOCAR PRIMITIVAS --> send, receive bloqueante, 
TCP IP se conversa for pela rede
tempo de synchronize

runtime orquestrador da execucao

mpirun inicia ssh no nó inicia programa, passa argumentos pra cada processo, e ai começa divisao dos dados.

% A arquitetura do \mfog é composta de múltiplos nós numa névoa que detectam
...
comunicação entre os processos de cada nó na névoa. Na névoa, cada nó possui um
conjunto de 4 processos, e cada processo possui seu espaço de endereçamento e é
...
O MPI é utilizado neste trabalho dentro de um programa monólito escrito em C, e utilizando a biblioteca do MPI.
Este monólito é executado por múltiplos processo dentro de cada nó. Cada processo recebe um conjunto de dados diferente. 
De acordo com o tipo de dado recebido, a processo executa funções diferentes. Se for um processo raiz, ele lê e distribui os dados de entrada
entre os nós folha. Já os nós folha executam todo cálculo de distância e detecção de novidades, portanto execução de forma paralela e distribuída.
Todos os processos (independente se estão no mesmo nó) se comunicam e são sincronizados através do OpenMPI. Portanto, é possível transferir um dado de qualquer nó para outro 
dentro da rede.

performance por causa do cache e do processamento, vc tem acesso a varios processadores e junto vem o cache que é amemoria mais rapida com computador e nao fica caindo tanto na memoria secundaria
 \cite{mpi-book}

 version usada foi mi 4.0.4
