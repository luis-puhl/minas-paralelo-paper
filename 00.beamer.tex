...
\begin{frame}[fragile]{Fundamentos - Processamento de Fluxos}
...
\begin{frame}[fragile]{Proposta - Implementação MPI}
  ...
  O \mfog é dividido em 2 módulos e 4 tarefas.
  
  \begin{itemize}
    \item \mfog foi implementado em linguagem C, OpenMPI 4.0.4, seguindo a técnica SPMD;
...
\begin{frame}
  \begin{figure}[h]
    \centering
    \includegraphics[width=0.85\textwidth,page=1]{figures/lifecycle-uml-svg.pdf}
    \vspace{-0.5em}
...
\begin{frame}{Resultados - Experimentos Adicionais}
  Tempo necessário para tratamento de cada descritor de fluxo admitido no sistema:
  \begin{figure}[h]
    \centering
    \begin{subfigure}{0.49\textwidth}
      \centering
...
\begin{frame}{Conclusão}
  ...
  \begin{itemize}
    \item Algoritmo \minas foi paralelizado e distribuído e a arquitetura \arch
    implementada com modificações;
    \item Distribuição tem pequeno efeito sobre as métricas de qualidade;
    \begin{itemize}
      \item Em especial, na versão distribuída há redução de etiquetas de
      novidade devido às filas de comunicação;
    \end{itemize}
