...
são limitados.
Para isso escolheu-se implementar a proposta utilizando o padrão \mpi e linguagem C,
onde tem-se controle fino sobre o uso de memória e distribuição.
% Não entendi a sentença a seguir... sugiro revisar ou retirar...
No entanto, perde-se facilidades e garantias inclusas nas plataformas
tradicionais de fluxos de dados que, por exemplo, oferecem o funcionamento confiável e contínuo com
mecânicas de recuperação de erros.

O algoritmo \minas original \cite{Faria2016minas} tem uma implementação
% companheira? Que tal "de referência" ?
...
previsto, provendo método de validação dos resultados e servindo também de comparação
...
\mclusters formando o modelo inicial é capturada em arquivo.
...
\begin{algorithm}[htb]
    ...
    \Function{\Mfog{fluxoEntrada, fluxoSaída}}{
        Modelo $\leftarrow$ $\emptyset$; trava $\leftarrow$ \textbf{new} \Lock()\; \label{line:init}
