...
\imprimiragradecimentos{
   ...
   Agradeço também ao CNPq pelo suporte \hlke{financiero} (contrato 167345/2018-4).
}
% Epígrafe
% \imprimirepigrafe{
...
\begin{resumo}{Detecção de Novidades. Detecção de Intrusão. Fluxos de Dados.
   ...
   (IoT), gerando um crescimento \hlpa{proporcional} no volume dos fluxos de dados
   gerados,
   ...
   % 
   No entanto, \hlpa{apesar de sua divisão em três partes} \hlpa{semi-independentes, este
   algoritmo ainda não foi adaptado para processar grandes} volumes de fluxos
   \hlpa{reais em ambiente de computação em névoa.}
...
\begin{abstract}{Novelty Detection. Intrusion Detection. Data Streams.
   ...
   environment, the impacts on the \hlke{accuracy,} and evaluate performance and
