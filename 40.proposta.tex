...
Executam-se múltiplas instâncias da tarefa de classificação do algoritmo \minas.
...
Após identificado o rótulo do descritor de fluxo (exemplo), este par (rótulo, exemplo)
\notaPA{Quem executa?}
pode ser utilizado imediatamente para tomada de decisões e, se o
exemplo é rotulado como ``desconhecido'', ele é armazenado para
\notaPA{Quem executa?}
...
\notaPA{Mas não aumenta o tempo para a classificação, caso haja diferentes padrões de fluxos de dados?}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    sincronização de modelo deve ter a propriedade de aditividade; também é preciso
    ...
    \notaPA{A propagação dos novos padrões
    é feita hierarquicamente, subindo
    da névoa para a nuvem, ou
    é propagada diretamente entre
    a porção em névoa?}
  ...
  ter mais de um exemplo no fluxo de saída, como ilustrado na Figura \ref{fig:marble}.
...
\begin{figure}[htb]
  \centerline{
    \includegraphics[width=0.9\linewidth,page=1]{figures/map-vs-flatmap.png}
  }
  \caption{Diagrama de bola de gude (\emph{marble diagram}) da operação de
  classificação: descarte \emph{versus} re-utilização de exemplos formadores de padrões novidade.}
  \label{fig:marble}
\end{figure}
