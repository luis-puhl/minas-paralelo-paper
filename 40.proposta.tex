...
temporal já que o número de clusters ... % falta algo aqui...

A fase \emph{offline} e a função de detecção de novidades do algoritmo \minas
...
% As decisões após essas discussões podem ser organizadas em várias políticas; alguns deles eram recorrentes durante os estudos para a implementação e são:
...
\begin{itemize}
  ...
  poderia ser mais acurado. % preciso (???)
...
a taxa de desconhecidos ($UnkR$ na \refequ{unkr}) \cite{Faria2013},
a acurácia média ($acc$ na \refequ{acc})
e o Macro F-score ($Fscore$ na \refequ{fscore}, também referido na literatura por
