...
Porém, como \streams variam temporalmente e são ilimitados,
...
\hl{sistema} que computa \streams a taxa de eventos
...
um único núcleo processador) e por isso, esses sistemas \hl{geralmente} são distribuídos.
...
\hl{onde} 
\hl{tem-se disponível o fluxo original}
\hl{com as etiquetas corretas e o fluxo resultante}
\hl{da classificação.}
...
A combinação de diferentes distribuições tem por objetivo \hl{demonstrar padrões de
...
A escolha desse conjunto para a implementação ocorreu \hl{devido à ampla}
